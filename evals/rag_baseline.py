"""Lexical RAG baseline over serialized operational rows.
...
no vector database, no API embeddings — so the comparison is deterministic
and cannot hide behind an unpaid embedding call.
"""
...
def score_rag_case(chunks: list[dict[str, Any]], case: dict[str, Any]) -> dict[str, Any]:
    sku = case.get("sku")
    if sku:
    ...
    for concept in expected_missing:
        if concept in present:
            errors.append(f"RAG invented {concept!r}")
    types = _object_types(chunks, sku)
    for needed in case.get("required_object_types") or []:
        if needed == "Recommendation":
            # Recommendations are engine output; the corpus does not contain them.
...
def run_rag_eval(con, *, k: int = DEFAULT_K) -> dict[str, Any]:
    corpus = build_corpus(con)
    results = []
    for case in QUESTIONS:
        hits = retrieve(case["question"], corpus, k=k)
        results.append(score_rag_case(hits, case))
