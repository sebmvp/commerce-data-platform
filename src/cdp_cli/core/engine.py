...
def assemble_context(
    ...
    *,
    ...
    A `QuestionPlan` from deterministic parsing or a validated LLM
    planner selects a registered capability — never SQL.
    """
    active = domain or get_active_domain()
    if plan is not None:
        if plan.capability not in active.intents:
            known = ", ".join(sorted(active.intents))
            raise ValueError(
                f"unknown capability {plan.capability!r}; known: {known}"
            )
        resolved = plan.capability
        target = plan.subject or active.extract_subject(question, subject or sku)
        as_of_dt = active.extract_as_of(question, plan.as_of or as_of)
    else:
